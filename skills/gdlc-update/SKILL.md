...
4. For `.claude/settings.json`: NEVER overwrite blindly. Read the consumer's existing `settings.json`, parse it as JSON, and operate at the **nested command level** — each event entry is a group (`{matcher?, hooks: [...]}`) whose `hooks` array can mix wizard and user commands. Replace only the nested commands whose `command` references the wizard's current hook script basename (`gdlc-prompt-check.sh`) with the command from the fetched template. Remove nested commands referencing a legacy basename from a prior wizard version (`instructions-loaded-check.sh`, the pre-v0.2.2 name; `gdlc-instructions-loaded-check.sh`, retired in v0.4.0 because Claude Code discards InstructionsLoaded hook stdout) wherever they appear, including under events the template no longer declares; drop a group only when its `hooks` array empties, and delete any event key left empty. Preserve every other nested command, group fields like `matcher`, and every other setting, then Write the result back. This in-skill merge mirrors `cli/init.js::mergeSettings` semantics and avoids the global force-apply pitfall.
5. **Legacy hook file removal.** After the settings merge, also remove any legacy wizard hook files left on disk: `.claude/hooks/instructions-loaded-check.sh` (pre-v0.2.2) and `.claude/hooks/gdlc-instructions-loaded-check.sh` (retired in v0.4.0). Both are unambiguously wizard-owned; leaving either dangling is the same DRIFT state `npx claude-gdlc-wizard check` flags. Use the Bash tool: `rm -f "$CLAUDE_PROJECT_DIR/.claude/hooks/instructions-loaded-check.sh" "$CLAUDE_PROJECT_DIR/.claude/hooks/gdlc-instructions-loaded-check.sh"` (silent if not present).

...
4. For `.claude/settings.json`: NEVER overwrite blindly. Read the consumer's existing `settings.json`, parse it as JSON, replace only the wizard hook entries — those whose `command` references one of the wizard's current hook script basenames (`gdlc-prompt-check.sh`, `gdlc-instructions-loaded-check.sh`) **OR** a legacy basename from a prior wizard version (`instructions-loaded-check.sh` — the pre-v0.2.2 name, before the SDLC-collision rename) — with the entries from the fetched template. Drop any legacy entries (don't carry both the legacy and the namespaced entry forward — that's the v0.2.1→v0.2.2 migration bug). Preserve every other field and every other hook entry, and Write the result back. This in-skill merge mirrors `cli/init.js::mergeSettings` semantics and avoids the global force-apply pitfall.
5. **Legacy hook file removal.** After the settings merge, also remove any `.claude/hooks/instructions-loaded-check.sh` file left on disk from a pre-v0.2.2 install. The file is unambiguously wizard-owned (legacy basename); leaving it dangling is the same DRIFT state `npx claude-gdlc-wizard check` flags. Use the Bash tool: `rm -f "$CLAUDE_PROJECT_DIR/.claude/hooks/instructions-loaded-check.sh"` (silent if not present).

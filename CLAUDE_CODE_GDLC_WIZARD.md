...
GDLC ships through `BaseInfinity/claude-gdlc-wizard`. Four parallel install channels — same surface (4 skills + 1 enforcement hook + 1 helper + settings.json + this wizard doc = 8 managed files) lands either way:
...
| `.claude/settings.json` | Installed by `npx claude-gdlc-wizard init` | Smart-merged — wizard hook entries updated via `--force`; user hooks and other settings preserved |
| `.claude/hooks/gdlc-prompt-check.sh` | Installed by `npx claude-gdlc-wizard init` | Update via `--force` |
| `.claude/hooks/_find-gdlc-root.sh` (sourced helper) | Installed by `npx claude-gdlc-wizard init` | Update via `--force` |
...
| step-0.2 | Verify wizard install | Run `npx claude-gdlc-wizard check` — full-surface verification (settings + 2 hook scripts + 4 skills + wizard doc + .gitignore = 9 rows) | No |
